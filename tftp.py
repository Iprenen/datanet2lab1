...
def tftp_transfer(fd, hostname, direction, filename):
    ...
    used_port = TFTP_LOSS_PORT30
  
    

    server_addr = socket.getaddrinfo(hostname, used_port)[0][4:][0] # Get server info, like IP and port
    ...
    sock.bind(("", used_port))
    sock.settimeout(TFTP_TIMEOUT) # Add timeout time to socket
    ...
    if direction == TFTP_GET: #We want to get a file
        ...
        print "Don't know if put or get."
        sys.exit(1)
    ...
    while True:
        if direction == TFTP_GET:
            ...
            try:
                ...
                if opcode == OPCODE_ACK and blocknr[0] == packetnr and total_file_sent != 1: # Check so it's the ack for the latest sent chunk
                ...
                elif opcode == OPCODE_ACK and blocknr[0] == packetnr and total_file_sent == 1:
